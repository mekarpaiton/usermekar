    ...
    return ChangeNotifierProvider(
      ...
      child: MaterialApp(
        ...
        theme: ThemeData(
          ...
          appBarTheme: const AppBarTheme(backgroundColor: warnaUtama, foregroundColor: Colors.white, elevation: 0, centerTitle: true),
        ),
...
class _SplashScreenState extends State<SplashScreen> with TickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: warnaUtama, body: Stack(children: [Positioned.fill(child: Image.asset('assets/images/splashmekar.png', fit: BoxFit.cover)), if (_isClicked) AnimatedBuilder(animation: _rippleController, builder: (context, child) => Center(child: CustomPaint(painter: ShockwavePainter(progress: _rippleController.value), size: const Size(200, 200)))), AnimatedBuilder(animation: _alignmentAnimation, builder: (context, child) => Align(alignment: _alignmentAnimation.value, child: Text(_isAtCenter?'📸':'📷', style: TextStyle(fontSize: 80))))]));
  }
}
class ShockwavePainter extends CustomPainter { final double progress; ShockwavePainter({required this.progress}); @override void paint(Canvas canvas, Size size) { final center = Offset(size.width/2, size.height/2); final paint1 = Paint()..color = const Color(0xff26a69a).withOpacity(1.0-progress)..style = PaintingStyle.stroke..strokeWidth = 4.0*(1.0-progress); canvas.drawCircle(center, progress*130, paint1); } @override bool shouldRepaint(covariant old) => old.progress!=progress; }
...
class _HalamanKatalogState extends State<HalamanKatalog> {
  ...
  static void shareProduk(BuildContext context, Map p) {
    ...
    Share.share("✨ ${p['nama']} - Rp $harga - ${AppConfig.namaToko}\n$linkPreview");
  }
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
                    ...
                    children: [
                      ...
                      Opacity(
                        ...
                        child: Padding(
                          ...
                          child: Text('SELAMAT DATANG DI TB MEKAR PAITON', style: TextStyle(fontSize: 11, letterSpacing: 1.2, color: warnaEmas, fontWeight: FontWeight.w600)),
                        ),
                      ),
                    ],
                  ),
          ...
          SliverToBoxAdapter(
            child: Container(width: double.infinity, padding: EdgeInsets.fromLTRB(16,16,16,12), decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 8)]), child: Column(children: [
              TextField(controller: searchCtrl, decoration: InputDecoration(hintText: 'Cari semen, cat, besi klasik...', prefixIcon: Icon(Icons.search, color: warnaUtama), filled: true, fillColor: warnaCream, border: OutlineInputBorder(borderRadius: BorderRadius.circular(30), borderSide: BorderSide.none)), onChanged: (v)=>getProduk(search: v, kat: kategoriDipilih)),
              SizedBox(height:12),
              SizedBox(height:38, child: ListView.builder(scrollDirection: Axis.horizontal, itemCount: kategori.length, itemBuilder: (ctx,i){ bool aktif=kategoriDipilih==kategori[i]; return Padding(padding: EdgeInsets.only(right:8), child: ChoiceChip(label: Text(kategori[i].toUpperCase(), style: TextStyle(fontSize: 11)), selected: aktif, selectedColor: warnaUtama, labelStyle: TextStyle(color: aktif?Colors.white:warnaUtama), onSelected: (s){ setState(()=>kategoriDipilih=kategori[i]); getProduk(search: searchCtrl.text, kat: kategori[i]); })); })),
            ])),
          ),
          SliverPadding(
            padding: EdgeInsets.all(14),
            sliver: loading? SliverToBoxAdapter(child: Center(child: CircularProgressIndicator(color: warnaUtama)))
            : produk.isEmpty? SliverToBoxAdapter(child: Center(child: Padding(padding: EdgeInsets.all(40), child: Text('Produk tidak ditemukan'))))
            : SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate((ctx,i){
                  ...
                  return GestureDetector(
                    onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (_)=>ProdukDetailPage(produk: p))),
                    child: Container(
                      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 12, offset: Offset(0,6))], border: Border.all(color: warnaEmas.withOpacity(0.15))),
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Expanded(flex:5, child: Stack(children: [
                          ...
                          if(isViral) Positioned(top:8, left:8, child: Container(padding: EdgeInsets.symmetric(horizontal:8, vertical:3), decoration: BoxDecoration(color: Colors.red[700], borderRadius: BorderRadius.circular(20)), child: Text('VIRAL', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold)))),
                        ...
                        Expanded(flex:4, child: Padding(padding: EdgeInsets.fromLTRB(10,10,10,8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Spacer(),
                          Text(hargaDisplay, style: TextStyle(color: warnaUtama, fontWeight: FontWeight.w900, fontSize:14)),
